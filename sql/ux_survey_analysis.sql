/* ============================================================
   GENHOUSE UX SURVEY ANALYSIS — SQL QUERIES (SQLite)
   Dataset: Early-Career UX Designer Survey Responses
   Table: survey_responses
   Notes: numeric fields are stored as TEXT → CAST for AVG/ordering
   ============================================================ */
...
SELECT AVG(CAST(NULLIF(portfolio_confidence_score, '') AS REAL)) AS avg_portfolio_confidence
FROM survey_responses;

-- Portfolio confidence by design level
SELECT design_level,
       AVG(CAST(NULLIF(portfolio_confidence_score, '') AS REAL)) AS avg_confidence
FROM survey_responses
GROUP BY design_level
...
SELECT CAST(NULLIF(standout_difficulty, '') AS INTEGER) AS standout_difficulty,
       COUNT(*) AS count
FROM survey_responses
GROUP BY CAST(NULLIF(standout_difficulty, '') AS INTEGER)
...
SELECT AVG(CAST(NULLIF(client_comfort_score, '') AS REAL)) AS avg_client_comfort
FROM survey_responses;

-- Client comfort by design level
SELECT design_level,
       AVG(CAST(NULLIF(client_comfort_score, '') AS REAL)) AS avg_client_comfort
FROM survey_responses
GROUP BY design_level
...
SELECT AVG(CAST(NULLIF(mentorship_value_score, '') AS REAL)) AS avg_mentorship_value
FROM survey_responses;

-- Mentorship value by mentorship type
SELECT mentorship_type,
       AVG(CAST(NULLIF(mentorship_value_score, '') AS REAL)) AS avg_value
FROM survey_responses
GROUP BY mentorship_type
...
SELECT AVG(CAST(NULLIF(platform_satisfaction, '') AS REAL)) AS avg_platform_satisfaction
FROM survey_responses;
...
SELECT CAST(NULLIF(recommend_if_interviews, '') AS INTEGER) AS recommend_if_interviews,
       COUNT(*) AS count
FROM survey_responses
GROUP BY CAST(NULLIF(recommend_if_interviews, '') AS INTEGER)
ORDER BY recommend_if_interviews DESC;
